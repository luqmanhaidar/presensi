<?php
/**
 * Site Paginating with MySQL Database Class for PHP 
 * Copyright (C) 2003 Cristian Ade Candra
 * 
 * Version  0.1.2  2005-01-11
 * 
 ...
 * 
 * Source code home page: http://www.creezz.com/cnn/cnnnav/
 * Contact author at: mail@creezz.com
 *
 * ChangeLog for version 0.1.2
 * Bug fixed for fetching data without any result records.
 *
 * ChangeLog for version 0.1.1  2004-03-15
 * Add class property CnnNav::_navStrOffset
 * 		Now you can use CnnNav more than one for each page.
 *		Just set different CnnNav::_navStrOffset.
 * 
 * ChangeLog for version 0.0.1  2003-10-15
 * 0.0.1 first release
 */
...
class CnnNav {
     ...
     */
	function CnnNav ($maxRows, $table,$extQuery="", $strFieldSelect = "*", $tableId = "", $numShow = "9", $interval = "5", $intervalShow = "3", $prev = "&laquo;prev", $next = "next&raquo;", $separator = " ", $css = "", $first = "&laquo;&laquo;", $last = "&raquo;&raquo;", $strOffset = "offset") {
	...
	function getPrev() {
		if ($this->_navOffset > 0) {
		    return "<a class=\"prevnext\" href=\"". $this->_navCurrentUri . $this->_navStrOffset ."=". ($this->_navOffset - 1) ."\">". $this->_retSpanTag($this->_navStrPrev) ."</a>"; 
		}
	}
	...
	function getNum() {
		if ($this->_navTotalPages > 1) {
			...
			return $strIntBefore . $strNumBefore . "<a class=\"curnav\" href=\"". $this->_navCurrentUri . $this->_navStrOffset . "=". ($this->_navOffset + 1 )."\">". $this->_retSpanTag($this->_navOffset + 1) ."</a>". $strNumAfter . $strIntAfter;
		} else return "";
	}
	...
	function getNext() {
		if ($this->_navOffset < ($this->_navTotalPages - 1) ) {
		    return "<a class=\"prevnext\" href=\"". $this->_navCurrentUri . $this->_navStrOffset . "=". ($this->_navOffset + 1) ."\">". $this->_retSpanTag($this->_navStrNext) ."</a>";
		}
	}
	 ...
	 **/
	function getResult(){
		$queryData = "SELECT ". $this->_navStrFieldSelect ." FROM ". $this->_navTable ." ".$this->_extQuery." LIMIT ". ($this->_navOffset * $this->_navMaxRows) .", ". $this->_navMaxRows;
