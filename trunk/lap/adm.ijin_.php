        ...
        switch ($_GET['cat'])
        {
                ...
                if(isset($_POST['submitAd']))
                {
                    ...
                    foreach ($sUser as $Nama)
                    {
                        ...
                        if($row=mysql_num_rows($SQL)>0){
                            echo "<script type=\"text/javascript\">alert('Gagal'); window.location =\"adm.ijin.php\";</script>";                            
                        }else{
                            $sql="INSERT INTO presensi (Nip,login,Tanggal,JamMasuk,Jamkeluar,abs,catatan) VALUES ('$nip','$nama','$Tanggal','00:00:00','00:00:00','4','$catatan')";
                            if($res=mysql_query($sql)){
                                echo "<script type=\"text/javascript\">alert('Insert Surat Ijin Pribadi Sukses'); window.location =\"adm.ijin.php\";</script>"; 
                             }else{
                                 echo "<script type=\"text/javascript\">alert('Gagal'); window.location =\"adm.ijin.php\";</script>";
                             }
                        }
                    ...
                    {
                        echo "<h1>Tambah Data Ijin Tidak Masuk (Ijin Pribadi)</h1>";
                        ...
                        $gol=fetchDistinct("profile","order by NamaLengkap");
            ...
            case    "edit"  :
                ...
                if(isset($_POST['submitEd']))
                {
                    ...
                    if($res=mysql_query($sql))
                    {
                        echo "<script type=\"text/javascript\">alert('Edit Data Ijin Pribadi Sukses'); window.location =\"adm.ijin.php\";</script>"; 
                    }
                    else{
                        echo "<script type=\"text/javascript\">alert('Edit Data Ijin Pribadi Gagal'); window.location =\"adm.ijin.php\";</script>";
                         }
                }else{
            ...
            case    "del"   :
                ...
                if($res=mysql_query($sql)){
                        echo "<script>alert('Hapus Ijin Sukses'); window.location=\"adm.ijin.php\"</script>";
                   }else{
                        echo "<script>alert('Hapus Ijin Gagal'); window.location=\"adm.ijin.php\"</script>";
                   }
                break;
            default         :
                echo "<h1>Managemen Ijin Pegawai (Ijin Pribadi) </h1>";
                ...
                $max=30;
                $i=1;
                $listnav=new CnnNav($max,"presensi","where abs <> '1' and abs <> '2' and abs <> '0'","*","id","9","5","3","&laquo;","&raquo;"," ", "");
                $start=$_GET['offset']?$_GET['offset']*$max:0;
                $nUser=fetchRow("profile,presensi","where profile.Nama=presensi.login AND presensi.abs <>'1' AND presensi.abs <> '2' AND abs <> '0' order by presensi.Tanggal desc limit ".$start.",".$max."");
                echo "<br />";
                if(!empty($nUser)){
                    echo "<table width=\"100%\" id=\"data\">";
                    echo "<tr><th>No</th><th><b>Nama Lengkap</b></th><th>Jabatan</th><th>Bagian / Fakultas</th><th>Tanggal</th><th>Keterangan</th><th>Edit</th><th>Hapus</th></tr>";
                    foreach($nUser as $u){
                        echo "<tr><td>$i</td><td>$u[NamaLengkap]</td><td>$u[Jabatan]</td><td>$u[Fak]</td><td>$u[Tanggal]</td><td>$u[catatan]</td><td><a href='adm.ijin.php?cat=edit&id=$u[id]'><img src='images/icon_calendar.gif'></a></td><td><a href=\"adm.ijin.php?cat=del&id=$u[id]\"><img src='images/icon_delete.gif'></a></td></tr>";
                    $i++;
                    }
                    echo "</table>";
                    echo "</p>";
