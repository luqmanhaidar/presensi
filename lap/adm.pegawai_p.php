...
switch ($_GET['cat']){
    ...
    case "edit" :
		...
		if($Nip !== "" && $Nama !== "" && $NamaLengkap !== "" && $Jab !== "" && $Jur !== "" && $Gol !== "" && $Fakk !== "" && $Status !== "" && $pass !== "")
			...
			$sql2="UPDATE profile SET Nip='$Nip',Nama='$Nama',NamaLengkap='$NamaLengkap',pass='$pass',Jabatan='$Jab',Fak='$Fakk',Jurusan='$Jur',Gol='$Gol',status='$Status' WHERE id='$id'";
			...
			if($pass==$cPass[pass]){
			    $sql3="UPDATE profile SET Nip='$Nip',Nama='$Nama',NamaLengkap='$NamaLengkap',pass='$pass',Jabatan='$Jab',Fak='$Fakk', Jurusan='$Jur',Gol='$Gol',status='$Status' WHERE id='$id'";
		...
		//echo "ok";
                ...
                echo "<label><b>User</b></label>";
                echo "<input name=\"Nama\" class=\"inputText\" type=\"text\" value=\"$Usr[Nama]\" />";
                echo "<label><b>Nama Lengkap</b></label>";
                ...
                echo "</p>";
                echo "<p class=\"right\">";
		//echo $_SESSION['userdata']['ulname'];
		echo "<label><b>Bagian</b></label>";
		if($_SESSION['userdata']['ulname']=='BAUK'){
		    echo "<select name=\"bagian\" class=\"inputText\">";
		    echo "<option value=\"UHTP\">UHTP</option>";
		    echo "<option value=\"KEUANGAN\">KEUANGAN</option>";
		    echo "<option value=\"KEPEGAWAIAN\">KEPEGAWAIAN</option>";
		    echo "</select>";
		}else{
		    echo "<select name=\"bagian\" class=\"inputText\">";
		    echo "<option>BAGAIAN PSI</option>";
		    echo "<option>KEMAHASISWAAN</option>";
		    echo "<option>DIKSAMA</option>";
		    echo "</select>";
		}
		...
		/**
                ...
                echo "</select>";
                **/
                echo "<label><b>Unit Kerja</b></label>";
                $jur=fetchDistinct("adm_jurusan","where Fakultas='".$_SESSION['userdata']['ulname']."'","JurId,JurDet");
                echo "<select name=\"Jur\" class=\"inputText\">";
                foreach ($jur as $j){
	...
	//$_jur=$_SESSION['userdata']['Jurusan'];
	echo "<br />".$fakk;
        $max=30;
        $i=1;
	$listnav=new CnnNav($max,"profile","where Jabatan='Karyawan' AND bag='".$fakk."'","*","id","9","5","3","&laquo;","&raquo;"," ", "");
	$start=$_GET['offset']?$_GET['offset']*$max:0;
        $nUser=fetchRow("profile","where Jabatan='Karyawan' AND bag='".$fakk."' order by Gol desc limit ".$start.",".$max."");
        if(isset($_POST['search'])){
            ...
            echo "<tr><th>No</th><th>Nama Lengkap</th><th>Nip</th><th>Jabatan</th><th>Bag / Jurusan</th><th>Last Login</th><th>Edit</th><th>Hapus</th></tr>";
                foreach($sql as $se){
                    echo "<tr><td>$i</td><td>$se[NamaLengkap]</td><td>$se[Nip]</td><td>$se[Jabatan]</td><td>$se[Jurusan]</td><td>$se[lastlogin]</td><td><a href='adm.pegawai_p.php?cat=edit&id=$se[id]'><img src='images/icon_calendar.gif'></a></td><td><a href='adm.pegawai.php?cat=del&id=$se[id]'><img src='images/icon_delete.gif'></a></td></tr>";
                    $i++;
                }
            echo "</table>";
            echo "</div>";
        ...
        foreach($nUser as $u){
            echo "<tr><td>$i</td><td>$u[NamaLengkap]</td><td>$u[Nip]</td><td>$u[Jabatan]</td><td>$u[Jurusan]</td><td><a href='adm.pegawai_p.php?cat=edit&id=$u[id]'><img src='images/icon_calendar.gif'></a></td><td><a href=\"adm.pegawai_p.php?cat=del&id=$u[id]\"><img src='images/icon_delete.gif'></a></td></tr>";
        $i++;
        }
        echo "</table>";
        echo "</p>";
