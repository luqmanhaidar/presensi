    ...
    switch ($_GET['cat'])
        {
                ...
                if(isset($_POST['submitAd']))
                {
                    ...
                    foreach ($sUser as $Nama)
                    {
                        ...
                        if($row=mysql_num_rows($SQL)>0){
                            echo "<script type=\"text/javascript\">alert('Insert Izin Gagal Yang Bersangkutan Telah Presensi masuk'); window.location =\"adm.ijin.php\";</script>";
                            ...
                            if($res=mysql_query($sql)){
                                echo "<script type=\"text/javascript\">alert('Insert Ijin Sukses'); window.location =\"adm.ijin.php\";</script>"; 
                             }else{
                                 echo "<script type=\"text/javascript\">alert('Insert Ijin Gagal'); window.location =\"adm.ijin.php\";</script>";
                             }
                        }
                    }                    
                }
                else
                    {
                        $bag= $_SESSION['userdata']['ulname'];
                                ...
                                $gol=fetchDistinct("profile","where Jabatan <> 'Mahasiswa' order by NamaLengkap");
            ...
            case    "edit"  :
                ...
                if(isset($_POST['submitEd']))
                {
                    ...
                    if($res=mysql_query($sql))
                    {
                        echo "<script type=\"text/javascript\">alert('Edit Data Ijin Sukses'); window.location =\"adm.ijin.php\";</script>"; 
                    }
                    else{
                        echo "<script type=\"text/javascript\">alert('Edit Data Ijin Gagal'); window.location =\"adm.ijin.php\";</script>";
                         }
                }else{
            ...
            default         :
                ...
                $nUser=fetchRow("profile,presensi","where profile.Nama=presensi.login AND presensi.abs <>'1' AND presensi.abs <> '5' AND presensi.abs <> '2' AND presensi.abs <> '0' order by presensi.Tanggal desc limit ".$start.",".$max."");                
                echo "<br />";
                if(!empty($nUser))
                    {
                        echo "<table width=\"100%\" id=\"data\">";
                        echo "<tr><th>No</th><th>Nama Lengkap</th><th>Fak/Bag</th><th>Tanggal</th><th>Alasan</th><th>Keterangan</th><th>Edit</th><th>Hapus</th></tr>";
                        foreach($nUser as $u){
                            echo "<tr><td>$i</td><td>$u[NamaLengkap]</td><td>$u[Fak]</td><td>$u[Tanggal]</td>";
                            switch ($u[abs]){
                                case "4" :
                            ...
                            echo "<td>$u[catatan]</td><td><a href='adm.ijin.php?cat=edit&id=$u[id]'><img src='images/icon_calendar.gif'></a></td><td><a href=\"adm.ijin.php?cat=del&id=$u[id]\"><img src='images/icon_delete.gif'></a></td></tr>";
                        $i++;
                        }
                        echo "</table>";
                        echo "</p>";
