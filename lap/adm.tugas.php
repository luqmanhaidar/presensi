        ...
        switch ($_GET['cat'])
        {
                ...
                if(isset($_POST['submitAd']))
                {
                    ...
                    foreach ($sUser as $Nama)
                    {
                        ...
                        if($res=mysql_query($sql)){
                            echo "<script type=\"text/javascript\">alert('Insert Ijin penugasan Sukses'); window.location =\"adm.tugas.php\";</script>"; 
                         }else{
                             echo "<script type=\"text/javascript\">alert('Gagal'); window.location =\"adm.tugas.php\";</script>";
                         }
                    }                    
                }
                else
                    {
                        echo "<h1>Tambah Data Ijin Penugasan</h1>";
            ...
            case    "edit"  :
                ...
                if(isset($_POST['submitEd']))
                {
                    ...
                    if($res=mysql_query($sql))
                    {
                        echo "<script type=\"text/javascript\">alert('Edit Data Ijin Penugasan Sukses'); window.location =\"adm.tugas.php\";</script>"; 
                    }
                    else{
                        echo "<script type=\"text/javascript\">alert('Edit Data Ijin Penugasan Gagal'); window.location =\"adm.tugas.php\";</script>";
                         }
                }else{
                    $gol=fetch("profile,presensi","WHERE profile.Nama=presensi.login AND presensi.id='".$id."'");
                
                    echo "<h1>Edit Data Ijin Tugas Atas Nama ".$gol[NamaLengkap]."</h1>";
                    echo "<div align=\"right\"><a href=\"".$_SERVER['PHP_SELF']."\"><img align=\"absmiddle\" style=\"border:0\" src=\"images/backs.png\" alt=\"Back\" border=\"0\" />&nbsp;Back</a></div>";
                    echo "<br />";
                    echo "<div>";
                    echo "<form method=\"post\" class=\"nice\">";
            ...
            case    "del"   :
                ...
                if($res=mysql_query($sql)){
                        echo "<script>alert('Hapus Ijin Penugasan Sukses'); window.location=\"adm.tugas.php\"</script>";
                   }else{
                        echo "<script>alert('Hapus Ijin Penugasan Gagal'); window.location=\"adm.tugas.php\"</script>";
                   }
                break;
            default         :
                ...
                $max=30;
                $i=1;
                $listnav=new CnnNav($max,"presensi","","*","id","9","5","3","&laquo;","&raquo;"," ", "");
                $start=$_GET['offset']?$_GET['offset']*$max:0;
                $nUser=fetchRow("profile,presensi","where profile.Nama=presensi.login AND presensi.abs='3' order by presensi.Tanggal desc limit ".$start.",".$max."");
                echo "<br />";
                if(!empty($nUser)){
                    ...
                    foreach($nUser as $u){
                        echo "<tr><td>$i</td><td>$u[NamaLengkap]</td><td>$u[Nip]</td><td>$u[Jabatan]</td><td>$u[Tanggal]</td><td>$u[catatan]</td><td><a href='adm.ijin.php?cat=edit&id=$u[id]'><img src='images/icon_calendar.gif'></a></td><td><a href=\"adm.ijin.php?cat=del&id=$u[id]\"><img src='images/icon_delete.gif'></a></td></tr>";
                    $i++;
                    }
                    echo "</table>";
                    echo "</p>";
