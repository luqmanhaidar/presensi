    ...
    switch ($_GET['cat'])
        {
            case    "new"   :
                ...
                if(isset($_POST['submitAd']))
                {
                        ...
                        if($row=mysql_num_rows($SQL) >0){
                           ...
                           //echo $sql;
                            if($res=mysql_query($sql)){
                                echo "<script type=\"text/javascript\">alert('Insert PRESENSI MANUAL Sukses'); window.location =\"presensi.manual.php\";</script>"; 
                             }else{
                                 echo "<script type=\"text/javascript\">alert('Insert PRESENSI MANUAL Gagal'); window.location =\"presensi.manual.php\";</script>";
                             }
                        }
                    ...
                    {                        
                        ...
                        switch($bag){
                            case    "BUPK"  :
                                $gol=fetchDistinct("profile","where Fak='BUPK' AND bag='".$fakk."' AND Jabatan <> 'Mahasiswa' order by NamaLengkap");
                                break;
                            case    "BAKI"   :
                                $gol=fetchDistinct("profile","where Fak='BAKI' AND bag='".$fakk."' AND Jabatan <> 'Mahasiswa' order by NamaLengkap");
                                break;
                            case    "WATES"   :
                        ...
                        echo "<input name=\"alasan\" type=\"radio\" value=\"5\"/> <b>Jaringan / Presensi Error</b><br />";
                        echo "<input name=\"alasan\" type=\"radio\" value=\"2\" CHECKED/> <b>Lupa Presensi</b><br />";
                        echo "<input name=\"alasan\" type=\"radio\" value=\"8\"/> <b>Salah Klik Presensi Pulang</b><br />";
                        echo "<br clear=\"all\" />";
            ...
            case    "edit"  :
                ...
                if(isset($_POST['submitEd']))
                {
                    ...
                    if($alasan ==''){
                        echo "<script type=\"text/javascript\">alert('Edit Data PRESENSI MANUAL Gagal Alasan Tidak Boleh Kosong'); window.location =\"presensi.manual.php\";</script>"; 
                    }else{
                        switch ($alasan){
                            case "5" :
                        ...
                        if($res=mysql_query($sql))
                        {
                            echo "<script type=\"text/javascript\">alert('Edit Data PRESENSI MANUAL Sukses'); window.location =\"presensi.manual.php\";</script>"; 
                        }
                        else{
                            echo "<script type=\"text/javascript\">alert('Edit Data PRESENSI MANUAL Gagal'); window.location =\"presensi.manual.php\";</script>";
                             }
                    }
                    ...
                    echo "<input name=\"alasan\" type=\"radio\" value=\"5\"/> <b>Jaringan / Presensi Error</b><br />";
                    echo "<input name=\"alasan\" type=\"radio\" value=\"2\" CHECKED/> <b>Lupa Presensi</b><br />";
                    echo "<input name=\"alasan\" type=\"radio\" value=\"8\"/> <b>Salah Klik Presensi Pulang</b><br /><br />";
            ...
            case    "del"   :
                ...
                if($res=mysql_query($sql)){
                        echo "<script>alert('Hapus PRESENSI MANUAL Sukses'); window.location=\"presensi.manual.php\"</script>";
                   }else{
                        echo "<script>alert('Hapus PRESENSI MANUAL Gagal'); window.location=\"presensi.manual.php\"</script>";
                   }
                break;
            default         :
                //$fakk=$_SESSION['userdata']['Fak'];
                ...
                if($_POST['Nama']){
                    ...
                    foreach($row as $u){
                                echo "<tr><td>$i</td><td>$u[NamaLengkap]</td><td>$u[Nip]</td><td>$u[JamMasuk]</td><td>$u[JamKeluar]</td><td>$u[Tanggal]</td><td><a href='presensi.manual.php?cat=edit&id=$u[id]'><img src='images/icon_calendar.gif'></a></td><td><a href=\"presensi.manual.php?cat=del&id=$u[id]\"><img src='images/icon_delete.gif'></a></td></tr>";
                            $i++;
                            }
                    echo "</table>";
                }else{
                    ...
                    switch($bag){
                        case    "BUPK"  :
                            $gol=fetchDistinct("profile","where Fak='BUPK' AND bag='".$fakk."' and Jabatan <> 'Mahasiswa' and Jabatan <> 'PKL' order by NamaLengkap");
                            break;
                        case    "BAKI"   :
                            $gol=fetchDistinct("profile","where Fak='BAKI' AND bag='".$fakk."' and Jabatan <> 'Mahasiswa' and Jabatan <> 'PKL' order by NamaLengkap");
                            break;
                        case    "WATES"   :
                            $gol=fetchDistinct("profile","where bag='WATES' and Jabatan <> 'Mahasiswa' and Jabatan <> 'PKL' order by NamaLengkap");
                            break;
                        default         :
                    ...
                    switch($bag){
                        case    "BUPK"  :
                            $nUser=fetchRow("profile,presensi","where profile.Nama=presensi.login AND presensi.JamMasuk is NULL AND profile.Fak='BUPK' AND profile.bag='".$fakk."' order by presensi.Tanggal desc limit ".$start.",".$max."");
                            break;
                        case    "BAKI"   :
                            $nUser=fetchRow("profile,presensi","where profile.Nama=presensi.login AND presensi.JamMasuk is NULL AND profile.Fak='BAKI' AND profile.bag='".$fakk."' order by presensi.Tanggal desc limit ".$start.",".$max."");
                            break;
                        case    "WATES"   :
                            $nUser=fetchRow("profile,presensi","where profile.Nama=presensi.login AND presensi.JamMasuk is NULL AND profile.bag='WATES' order by presensi.Tanggal desc limit ".$start.",".$max."");
                            break;
                        default         :
                    ...
                    if(!empty($nUser))
                        {
                            ...
                            foreach($nUser as $u){
                                echo "<tr><td>$i</td><td>$u[NamaLengkap]</td><td>$u[Nip]</td><td>$u[JamMasuk]</td><td>$u[JamKeluar]</td><td>$u[Tanggal]</td><td><a href='presensi.manual.php?cat=edit&id=$u[id]'><img src='images/icon_calendar.gif'></a></td></tr>";
                            $i++;
                            }
                            echo "</table>";
                            echo "</p>";
